...
var HeartbeatAscoltatore = function(ascoltatore, id, heartbeat, deathTime){
    ...
    this._nodeChecker();
    // replicate ascoltatori behaviour
    var that = this;
};
...
HeartbeatAscoltatore.prototype.test = function(){
    this.ascoltatore.emit('test');
}
...
HeartbeatAscoltatore.prototype._encapsulateCallback = function(channel, message, callback){
...
HeartbeatAscoltatore.prototype.subscribe = function(channel, callback, done){
    var that = this;
    this.ascoltatore.subscribe(channel, function(channel, message){
        that._encapsulateCallback(channel, message, callback);
    }, done);
};
...
HeartbeatAscoltatore.prototype.unsubscribe = function(channel, callback, done){
    var that = this;
    this.ascoltatore.unsubscribe(channel, function(channel, message){
        that._encapsulateCallback(channel, message, callback);
    }, done);
};
