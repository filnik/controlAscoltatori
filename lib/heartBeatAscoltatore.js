...
var HeartbeatAscoltatore = function (opts) {
  ...
  this.statsChannel = opts.statsChannel || 'control';
  this.id = opts.id || uuid.uuid.v1();
  // for the close
...
/**
 * parse the events, to redirect correctly the events
 *
 * @param channel to parse
 * @param callback to call
 * @return {Function} to execute
 * @private
 */
HeartbeatAscoltatore.prototype._parseControlChannel = function (channel, callback) {
  var that = this;
  return function (message) {
    if (channel !== 'newTopic' || message.indexOf(that.statsChannel + '/') === -1) {
      callback(message);
    }
  };
};
...
HeartbeatAscoltatore.prototype.on = function (channel, callback) {
  var that = this;
  // filter out the newTopic event for the control channel
  this.ascoltatore.on(channel, this._parseControlChannel(channel, callback));
};
...
HeartbeatAscoltatore.prototype._heartBeat = function () {
  ...
  this.sendStats();
  that.ascoltatore.emit('newControlTopic');
